...
if docker cp "$CONTAINER_NAME:/home/agent/.codex" "$SESSION_EXTRACT_DIR" 2>/dev/null; then
    ...
    # Extract commands from Codex JSONL format
    if [[ -n "$JSONL_FILE" && -f "$JSONL_FILE" ]]; then
        CMD_LOG="$WORKSPACE_DIR/logs/agent-commands-executed.log"
        write_command_log_header "$CMD_LOG" "$JSONL_FILE"

        # Parse Codex JSONL: function_call with shell_command (wrapped in payload)
        jq -r '
            select(.type == "response_item" and .payload.type == "function_call" and .payload.name == "shell_command") |
            .payload.arguments | fromjson |
            "$ " + .command + (if .workdir then " # (in " + .workdir + ")" else "" end)
